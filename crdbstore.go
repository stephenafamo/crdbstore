...
// NewCrDBStore creates a new CrDBStore instance and a new database/sql pool.
// This will also create in the database the schema needed by CrDBStore.
func NewCrDBStore(dbURL string, keyPairs ...[]byte) (*CrDBStore, error) {
	...
	return NewCrDBStoreFromPool(db, keyPairs...)
}

// NewCrDBStoreFromPool creates a new CrDBStore instance from an existing
// database/sql pool.
// This will also create the database schema needed by CrDBStore.
...
// The default for a new CrDBStore is 4096. PostgreSQL allows for max
...
// into session.Values.
func (db *CrDBStore) load(session *sessions.Session) error {
	var s CrDBSession
...
func (db *CrDBStore) save(session *sessions.Session) error {
...
// Delete session
func (db *CrDBStore) destroy(session *sessions.Session) error {
	_, err := db.DbPool.Exec("DELETE FROM http_sessions WHERE key = $1", session.ID)
	return err
}

func (db *CrDBStore) createSessionsTable() error {
...
func (db *CrDBStore) selectOne(s *CrDBSession, key string) error {
...
func (db *CrDBStore) insert(s *CrDBSession) error {
...
func (db *CrDBStore) update(s *CrDBSession) error {
