...
func (db *CrDBStore) cleanup(interval time.Duration, quit <-chan struct{}, done chan<- struct{}) {
	...
	for {
		...
		case <-ticker.C:
			...
			if err != nil {
				log.Printf("crdbstore: unable to delete expired sessions: %v", err)
			}
		}
	}
}
...
func (db *CrDBStore) deleteExpired() error {
